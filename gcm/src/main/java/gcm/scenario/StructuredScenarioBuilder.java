 ...
 * {@link UnstructuredScenarioBuilder} or {@link ExperimentBuilder} classes
 * instead.
 *
...
public final class StructuredScenarioBuilder implements ScenarioBuilder {
	...
	private final static class ScenarioData {
		...
		private final Map<GlobalComponentId, Class<? extends Component>> globalComponentIds = new LinkedHashMap<>();

		private final Map<MaterialsProducerId, Class<? extends Component>> materialsProducerIds = new LinkedHashMap<>();
		...
		public int hashCode() {
			...
			result = prime * result + ((suggestedPopulationSize == null) ? 0 : suggestedPopulationSize.hashCode());
			return result;
		}
		...
		public boolean equals(Object obj) {
			...
			if (suggestedPopulationSize == null) {
				if (other.suggestedPopulationSize != null) {
					return false;
				}
			} else if (!suggestedPopulationSize.equals(other.suggestedPopulationSize)) {
				return false;
			}
			return true;
		}
	...
	private final static class ScenarioImpl implements Scenario {
		...
		public ScenarioId getScenarioId() {
			return scenarioData.scenarioId;
		}
		...
		public int getSuggestedPopulationSize() {
			Integer result = scenarioData.suggestedPopulationSize;
			if (result == null) {
				result = 0;
			}
			return result;
	...
	private static void validateMaterialsProducerPropertyNotSet(final ScenarioData scenarioData, final MaterialsProducerId materialsProducerId,
	...
	private static void validateScenarioId(final ScenarioId scenarioId) {
		if (scenarioId == null) {
			throwNullInputException(ScenarioErrorType.NULL_SCENARIO_ID);
		}
		if (scenarioId.getValue() < 1) {
			throwNegativeValueException(ScenarioErrorType.NON_POSITIVE_SCENARIO_ID, scenarioId);
		}
	}

	private static void validateScenarioIdNotSet(final ScenarioData scenarioData) {
		if (scenarioData.scenarioId != null) {
			throwPreviouslyAssignedValueException("Scenario Id");
		}
	}
	 ...
	 */
	private static void validateValueCompatibility(final ScenarioData scenarioData, ActionType actionType, final Object propertyId, final PropertyDefinition propertyDefinition,
	...
	private static void validateComponentClass(Class<? extends Component> componentClass) {
		if (componentClass == null) {
			throwNullComponenClassException();
		}
	}

	private static void throwNullComponenClassException() {
		throw new ScenarioException(ScenarioErrorType.NULL_COMPONENT_CLASS);
	}
	...
	public void addCompartmentId(final CompartmentId compartmentId, Class<? extends Component> comppartmentComponentClass) {
		...
		try {
			...
			validateComponentClass(comppartmentComponentClass);
			scenarioData.compartmentIds.put(compartmentId, comppartmentComponentClass);
		} finally {
			releaseLock();
		}
	}
	...
	public void addGlobalComponentId(final GlobalComponentId globalComponentId, Class<? extends Component> globalComponentClass) {
		...
		try {
			...
			scenarioData.globalComponentIds.put(globalComponentId, globalComponentClass);
		} finally {
			releaseLock();
		}
	}
	...
	public void addMaterialsProducerId(final MaterialsProducerId materialsProducerId, Class<? extends Component> materialProducerComponentClass) {
		...
		try {
			...
			validateComponentClass(materialProducerComponentClass);
			scenarioData.materialsProducerIds.put(materialsProducerId, materialProducerComponentClass);
		} finally {
			releaseLock();
		}
	}
	...
	public void addRegionId(final RegionId regionId, Class<? extends Component> regionComponentClass) {
		...
		try {
			...
			validateComponentClass(regionComponentClass);
			scenarioData.regionIds.put(regionId, regionComponentClass);
		} finally {
			releaseLock();
		}
	...
	public Scenario build() {
		...
		try {
			...
			if (scenarioData.scenarioId == null) {
				scenarioData.scenarioId = new ScenarioId(0);
			}
	...
	public void setScenarioId(final ScenarioId scenarioId) {
		...
		try {
			...
			validateScenarioIdNotSet(scenarioData);
	...
	private static void validateSuggestedPopulationSize(final Integer suggestedPopulationSize) {
		if (suggestedPopulationSize == null) {
			throwNullInputException(ScenarioErrorType.NULL_SUGGESTED_POPULATION_SIZE);
		}
		if (suggestedPopulationSize < 0) {
			throwNegativeValueException(ScenarioErrorType.NEGATIVE_SUGGGESTED_POPULATION, suggestedPopulationSize);
		}
	}

	private static void validateSuggestedPopulationSizeNotSet(final ScenarioData scenarioData) {
		if (scenarioData.suggestedPopulationSize != null) {
			throwPreviouslyAssignedValueException("Suggested Population Size");
		}
	}
	...
	public void setSuggestedPopulationSize(int suggestedPopulationSize) {
		...
		try {
			...
			validateSuggestedPopulationSizeNotSet(scenarioData);
			scenarioData.suggestedPopulationSize = suggestedPopulationSize;
		} finally {
			releaseLock();
		}
