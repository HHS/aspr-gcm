...
public interface PersonGroupManger extends Element {
	...
	/**
	 * Associates the person with the group. The group id must be valid. The
	 * person id must be valid. The person must not already be a member of the
	 * group.
	 */
	 ...
	 * cannot be null.
	 */
	public StochasticPersonSelection sampleGroup(final GroupId groupId, final BiWeightingFunction biWeightingFunction, final PersonId sourcePersonId, final boolean excludeSourcePerson);
	...
	/**
	 * Returns a list of unique groupIds associated with the given group type
	 * Id. Group type id must be valid.
	 *
	 */
	public List<GroupId> getGroupsForGroupType(final GroupTypeId groupTypeId);

	/**
	 * Returns a list of the unique group ids associated with the given person
	 * and group type. Group type id must be valid. Person id must be non-null
	 * and non-negative.
	 */
	public List<GroupId> getGroupsForGroupTypeAndPerson(final GroupTypeId groupTypeId, final PersonId personId);

	/**
	 * Returns the list of unique groups associated the the given person. The
	 * person id must be non-null and non-negative.
	 */
	 ...
	 * group memberships. The person id must be non-null and non-negative.
	 */
	public int getGroupTypeCountForPersonId(final PersonId personId);

	/**
	 * Returns the list of unique group types associated with the person's
	 * groups.
	 */
	...
	/**
	 * Returns a contacted person. The group should exist. The
	 * MonoWeightingFunction must not be null. Uses the random generator
	 * associated with the RandomNumberGeneratorId.
	 */
	public StochasticPersonSelection sampleGroup(final GroupId groupId, final MonoWeightingFunction monoWeightingFunction, RandomNumberGeneratorId randomNumberGeneratorId);
	...
	/**
	 * Returns the number of people in the given group. The group id must be
	 * valid.
	 */
	public int getPersonCountForGroup(final GroupId groupId);

	/**
	 * Returns the number of people(by unique count) who are associated with
	 * groups having the given group type. The group type id must be valid.
	 */
