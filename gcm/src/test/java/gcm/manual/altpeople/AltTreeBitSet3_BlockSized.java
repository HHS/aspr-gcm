 ...
 * 
 * Block size is now chosen in construction and does not have to be a power of 2
 * 
 * @author Shawn Hatch
 */
@Source(status = TestStatus.REQUIRED, proxy = EnvironmentImpl.class)
public class AltTreeBitSet3_BlockSized implements AltPeopleContainer {
	...
	// power
	private TreeHolder treeHolder = new TreeHolder(1 << power);
		...
		if (value >= maxPid) {
			...
			for (int p = 0; p < power; p++) {
				for (int i = 0; i < base; i++) {
					...
					newTreeHolder.set(newBase + i, treeHolder.get(i + base));
				}
				base <<= 1;
				newBase <<= 1;
			}
	...
	public int getValue(int index) {
		...
		int midTreeIndex = treeHolder.length() >> 1;
		int treeIndex = 1;
